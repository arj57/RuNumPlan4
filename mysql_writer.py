...
class MysqlWriter(AbstractWriter):
    def __init__(self, config: config.Config) -> None:
        ...
        self.fields_types: list[str] = self.__get_fields_types()

        conn_params = self.__get_connection_parameters()
    ...
    def __get_connection_parameters(self) -> datatypes.DbConnectionParameters:
        res: datatypes.DbConnectionParameters = {
    ...
    def get_filelist(self) -> typing.Optional[list[str]]:
        logger.info("Получаем список ранее обработанных файлов из БД...")
        cursor = self.conn.cursor()
        try:
            cursor.execute("SELECT loaded_file FROM metadata ORDER BY loaded_file DESC")
            rows = list(map(lambda x: x[0], cursor.fetchall()))
        except mysql.connector.Error as er:
            logger.error('Ошибка при получении списка: %s' % er)
            raise IOError(er)
        finally:
            cursor.close()
        logger.debug("Получили. (Три последних: %s )" % rows[:3])
        return rows

    @override()
    def put_data(self, data2: datatypes.InpData) -> None:
        cursor = self.conn.cursor()
        fields_list: list[str] = self.get_fieldslist()
    ...
    def __get_fields_types(self) -> list[str]:
    ...
    def __dicts_to_tuples(self, data: datatypes.InpRecords) -> list[datatypes.TupRec]:
        """
        Typecasting of each field of record according to config
        """

        # r: datatypes.Records = data['cdr_records']
        def record_typecasting(str_rec: datatypes.InpRecord) -> datatypes.TupRec:
...
class MysqlMetadata(AbstractMetadata):
    def __init__(self, data: datatypes.InpData, connection: mysql.connector.MySQLConnection):
        super().__init__(data)
    ...
    @override()
    def save(self) -> None:
        metadata: datatypes.TMetadata = self.get_metadata()
